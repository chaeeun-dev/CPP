                     ...
                     _In_ int       nCmdShow)
{
    ...
    Game game;
    game.Init(g_hWnd);  // 전역 변수, InitInstace 함수에서 받은 값을 넣어줌
    ...
    {
        if (::PeekMessage(&msg, NULL, 0, 0, PM_REMOVE)) // 기본적으로 처리
        {
            ::TranslateMessage(&msg);
            ::DispatchMessage(&msg);
        }
        else // 기본 처리 후 실행할 코드 실행
        {
...
BOOL InitInstance(HINSTANCE hInstance, int nCmdShow)
{
   ...
   // 넣어준 값을 가지고 창 크기 설정, 메뉴바 제외한 실제 크기

   // 핵심 - CereateWindowW 함수 : 창을 만들어준다
